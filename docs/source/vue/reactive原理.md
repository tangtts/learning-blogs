# reactive原理
:::info
  **首先要使用 `reactive` 属性,必须要在 `effect` 函数内部, 因为 `reactive` 属性是响应式的, 只有响应式才能被 `effect` 函数追踪**
:::
## 1. 响应式原理
<red>本质是发布订阅模式</red>
...
```
其中在 `mutableHandlers` 会使用 `get/set` 访问器属性代理访问源对象，稍后再说

## effect
effect 接受一个函数,在函数内部使用 `reactive` 属性,并且 `effect` 要自动执行一次,当 `reactive` 的属性发生变化时,会再次自动执行函数    

**所以要把 `effect` 与 `reactive` 属性关联起来**

定义全局变量 `activeEffect` 不断的指向执行的  `effect`,`effect` 有可能是嵌套,所以要记录父级`effect`
```js
...
当执行 `_effect.run()` 时,会执行 `fn` 函数,会触发 `reactive` 的 `get` 访问器
### 依赖收集 get / set
...
```
使用 `weakMap` 收集 `target`,在 `target` 上的 `key` 对应的 `set` 中添加 `activeEffect`,同时 `activeEffect.deps` 也收集 `dep`  
