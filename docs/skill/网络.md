...
使用等号分隔键和值，并使用  `&` 分隔不同字段，然后对特殊字符进行编码（例如空格变为 %20）。这种格式适合传输简单的键值对，并且在 HTML **表单的默认提交** 行为中使用  
...
这是一种以多部分形式编码的数据格式，通常用于上传文件或包含二进制数据的表单。它允许同时传输多个键值对以及文件数据，每个字段都有自己的 Content-Disposition 标头和相应的数据。这种格式通常用于使用 FormData 对象发送数据，可以通过 JavaScript 将表单数据构造为 FormData 对象并进行提交
